...
{  
  ...
  RandomPermutation x = new RandomPermutation();
  ...
  public Board()
  { 
    int[] randRow = x.next(3);
  
    int[] randCol = x.next(4);
    for(int i = 0; i < randRow.length;i++){
      randRow[i]--;
    }
    for(int z = 0; z < randCol.length;z++){
      randCol[z]--;
    }
    int iterateY = 0;
    for(int i =0;i <= 2; i++){
        for(int z = 0; z<=3;z++){
            gameboard[randRow[i]][randCol[z]] = new Tile(tileValues[iterateY]);
            iterateY++;
        }
      }
 ...
 /** 
  ...
  {
    int nextline = 0;
    int limit = gameboard[0].length;
    for (Tile[] row: gameboard){
      for (Tile str : row){
      if(nextline == (limit-1)){
        if(str.isShowingValue())
        System.out.println(str);
        else
        System.out.println(str.getHidden());
      ...
      System.out.print(str.getHidden() + "\t");
